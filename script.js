...
function drawFirework() {
    ...
    if(rocket.active) {
        ...
        if(rocket.y <= rocket.tY) { 
            rocket.active = false; 
            for(let i=0; i<100; i++) particles.push({ x: rocket.x, y: rocket.y, angle: Math.random()*Math.PI*2, speed: Math.random()*5+2, a: 1 });
...
function startTypewriter() {
    const text = "Happy New Year Anurag! May 2026 be kind, exciting, and full of opportunities 🌟";
